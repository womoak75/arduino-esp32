...
#define MBEDTLS_TS_DEBUG 1

#ifdef MBEDTLS_TS_DEBUG
#define MBEDTLS_TS_DEBUG_PRINTF(...)  { printf("%s:%d ",  __FILE__, __LINE__); printf(__VA_ARGS__); printf("\n"); }
...
int mbedtls_ts_get_signer_info(unsigned char **p, unsigned char * end, mbedtls_asn1_ts_pki_signer_info * signer_info)
{
  ...
  switch (sigVersion) {
    case 1:
...
#ifdef MBEDTLS_TS_DEBUG
      {
        ...
        MBEDTLS_TS_DEBUG_PRINTF("SID name %s", s);
      };
#endif
  ...
  CHKR(mbedtls_x509_get_algorithm_itentifiers(p, *p + len, (int*) & (out.sig_digest_type)));
      ...
      *p += len;
      ...
      if ( MBEDTLS_OID_CMP( MBEDTLS_OID_PKCS9_MESSAGE_DIGEST, &oid ) == 0 ) {
        ...
        */
        ...
        CHKR(mbedtls_x509_get_algorithm_itentifiers(p, evap, (int *)&md_alg));
  ...
  CHKR(mbedtls_x509_get_algorithm_itentifiers(p, *p + len, (int *) & (out.sig_type)));
